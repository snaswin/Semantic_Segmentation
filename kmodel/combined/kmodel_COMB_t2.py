...
#################################
input_dir = main_dir_1 + "/X/"
target_dir = main_dir_1 + "/Y/"
mx1 = glob.glob(input_dir + "/*.png")
my1 = glob.glob(target_dir + "/*.png")

input_dir = main_dir_2 + "/X/"
target_dir = main_dir_2 + "/Y/"
mx2 = glob.glob(input_dir + "/*.png")
my2 = glob.glob(target_dir + "/*.png")

mx = sorted(mx1) + sorted(mx2)
my = sorted(my1) + sorted(my2)

inds = np.arange(len(mx))
np.random.seed(7)
np.random.shuffle(inds)
inds = inds[:fetch_size]

input_img_paths = np.take(mx, inds).tolist()
target_img_paths = np.take(my, inds).tolist()


print("Num of ims- ", len(input_img_paths))

for ipath, tpath in zip(input_img_paths[:50], target_img_paths[:50]):
	print(ipath, " & ", tpath)
